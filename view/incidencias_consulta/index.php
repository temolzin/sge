    ...
    $menu = new Menu();
    $menu->header('incidencias_consulta');
    ?>
          ...
          <div class="col-12">
            <div class="card">
             <div class="card-header">
                        <h3 class="card-header">Incidencias</h3>
                    </div>
              <!-- /.card-header -->
              <div class="card-body">
 ...
 <script>
  ...
  var mostrarAlumnoIncidencia = function() {
    var TableAlumnoIncidencia = $('#dataTableAlumnoIncidencia').DataTable({
      "processing": true,
      "ajax": {
        "url": "<?php echo constant('URL');?>incidencias_consulta/readTable"
      },
      "columns": [
        { defaultContent: "",
        ...
        {data:null,
          "defaultContent":
          `<button class='consulta btn btn-primary' data-toggle='modal' data-target='#modalDetalleAlumnoIncidencia' title="Ver Detalles"><i class="fa fa-eye"></i></button>`
        }
        ],
        responsive: true,
        autoWidth: false,
  ...
  var obtenerdatosDT = function (table) {
    $('#dataTableAlumnoIncidencia tbody').on('click', 'tr', function() {
      ...
      var id_alumnoonsulta = $("#id_alumnoConsultar").val(data.nombre_alumno + ' ' + data.appaterno_alumno + ' ' + data.apmaterno_alumno);
